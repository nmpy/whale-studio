// src/app/_components/owner-dashboard/owner-dashboard-section.tsx
// スタジオ全体ダッシュボードの共有表示セクション（Server Component）。
//   - /oas（platform owner のトップ）と、必要なら /admin 互換表示の双方から使える。
//   - お知らせ帯は含めない（呼び出し側＝アカウント一覧で1回だけ表示するため重複させない）。
//   - 期間切替・エラー詳細開閉・アクティビティ表示は既存の Client/Server 部品を再利用（移動しない）。
//   - showAccountSummary=false のとき「アカウント別サマリー表」を出さない（/oas では直下に本来の一覧があるため）。

import Link from "next/link";
import { accountColor, type OwnerDashboardData, type DashboardPeriod } from "@/lib/owner-dashboard/aggregate";
import type { OwnerActivityItem } from "@/lib/owner-dashboard/activity";
import { PeriodSelect } from "../../admin/dashboard/_period-select";
import { ErrorRateCard } from "../../admin/dashboard/_error-rate-card";
import { ActivitySection } from "../../admin/dashboard/_activity-section";
...
/**
 * @param accountListHref アカウント一覧への導線 URL。null のとき導線リンクを出さない
 *   （/oas では直下に一覧があるため不要）。
 * @param showAccountSummary アカウント別サマリー表を出すか（/oas では false）。
 */
export function OwnerDashboardSection({
  data, activity, period, showAccountSummary = true, accountListHref = "/oas",
}: {
  data: OwnerDashboardData;
  activity: OwnerActivityItem[];
  period: DashboardPeriod;
  showAccountSummary?: boolean;
  accountListHref?: string | null;
}) {
  ...
  return (
    <section aria-label="スタジオ全体ダッシュボード">
      {/* タイトル行 */}
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <h2 className="font-round text-[20px] font-bold text-ink">スタジオ全体ダッシュボード</h2>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {accountListHref && (
            <Link href={accountListHref} className="rounded-full border border-line bg-surface px-3 py-1.5 text-[12px] font-semibold text-ink-2 transition-colors hover:border-brand hover:text-brand-ink">アカウント一覧へ ›</Link>
          )}
        </div>
      </div>

      {/* サマリー 6 枚。エラー率カードは開くと下に詳細パネル（col-span-full）。 */}
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 lg:grid-cols-[1.4fr_1fr]">
        <div className="rounded-[14px] border border-line bg-surface p-5 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-[13px] font-bold text-ink">直近{daily.length}日間のプレイヤー数（全アカウント合計）</h3>
        ...
        <div className="rounded-[14px] border border-line bg-surface p-5 shadow-sm">
          <h3 className="mb-3 text-[13px] font-bold text-ink">プレイヤー内訳（アカウント別）</h3>
          {accountBreakdown.length === 0 || breakdownTotal === 0 ? (
            ...
            <div className="flex flex-col gap-2.5">
              ...
              {accountListHref && accountBreakdown.length > 8 && <Link href={accountListHref} className="mt-1 text-[12px] font-semibold text-brand-ink hover:underline">すべて見る（{accountBreakdown.length}件）›</Link>}
            </div>
          )}
        </div>
      </div>

      {/* アカウント別サマリー表（/oas では直下に本来の一覧があるため非表示） */}
      {showAccountSummary && (
        <section className="mb-4 overflow-hidden rounded-[14px] border border-line bg-surface shadow-sm">
          <div className="flex items-center justify-between px-5 py-3.5">
            <div className="flex items-center gap-2">
              <h3 className="text-[13px] font-bold text-ink">アカウント別サマリー</h3>
              <span className="rounded-full border border-line bg-bg-tint px-2 py-0.5 text-[11px] text-ink-3">{accountTable.length}件</span>
            </div>
            {accountListHref && <Link href={accountListHref} className="text-[12px] font-semibold text-brand-ink hover:underline">アカウント一覧へ ›</Link>}
          </div>
