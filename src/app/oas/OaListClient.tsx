...
import { oaApi, workApi, getDevToken, type OaListItem, type OaListMeta } from "@/lib/api-client";
...
type OaListClientProps = {
  initialItems: OaListItem[];
  initialMeta: OaListMeta;
  initialWorksMap: Record<string, OaListWorkItem[]>;
  initialError?: string | null;
};

export type OaListWorkItem = {
  id: string;
  title: string;
...
/* ── 統計サマリー ────────────────────────────────────────────────────────── */
function SummaryBar({ items, worksMap }: { items: OaListItem[]; worksMap: Record<string, OaListWorkItem[]> }) {
...
export function OaListClient({
  ...
  const [items,        setItems]        = useState<OaListItem[]>(initialItems);
  const [meta,         setMeta]         = useState<OaListMeta | null>(initialMeta);
  const [loading,      setLoading]      = useState(false);
  const [worksLoading, setWorksLoading] = useState(false);
  const [error,        setError]        = useState<string | null>(initialError);
  const [page,         setPage]         = useState(1);
  const [worksMap,     setWorksMap]     = useState<Record<string, OaListWorkItem[]>>(initialWorksMap);
  ...
  const hideOwnerActions = isPlatformOwner && effectiveRole !== "owner";
      ...
      const pairs = await Promise.all(
        result.data.map((oa) =>
          workApi.list(token, oa.id)
            .then((ws) => [oa.id, ws] as [string, OaListWorkItem[]])
            .catch(() => [oa.id, [] as OaListWorkItem[]] as [string, OaListWorkItem[]])
        )
      );
      const map: Record<string, OaListWorkItem[]> = {};
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flexShrink: 0 }}>
                    <RowActions
                      oaId={oa.id}
                      isOwner={oa.my_role === "owner" && !hideOwnerActions}
