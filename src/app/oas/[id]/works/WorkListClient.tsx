...
import { useWorkLimit, type InitialWorkLimitState } from "@/hooks/useWorkLimit";
...
type WorkListClientProps = {
  oaId: string;
  initialOaTitle: string;
  initialWorks: WorkListItem[];
  initialFriendAdd: FriendAddSettings | null;
  initialWorkLimit: InitialWorkLimitState;
  initialWorkspaceRole: Role | null;
...
export function WorkListClient({
  ...
  initialFriendAdd,
  initialWorkLimit,
  initialWorkspaceRole,
  initialError = null,
}: WorkListClientProps) {
  const { showToast } = useToast();
  const sp = useIsMobile();
  const { role, isTester: isRoleTester } = useWorkspaceRole(oaId, initialWorkspaceRole);
  const { isTester } = useTesterMode();
  const { maxWorks, planDisplayName, planName, loading: limitLoading } = useWorkLimit(oaId, initialWorkLimit);

  const [oaTitle, setOaTitle]     = useState(initialOaTitle);
  const [works, setWorks]         = useState<WorkListItem[]>(initialWorks);
  const [friendAdd, setFriendAdd] = useState<FriendAddSettings | null>(initialFriendAdd);
