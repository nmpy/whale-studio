...
type ScenarioClientProps = {
  ...
  initialPhases: PhaseWithCounts[];
  initialTransitions: TransitionWithPhases[];
  initialMessages: Message[];
  initialMsgQrData: Record<string, MsgQrEntry[]>;
};
...
export function ScenarioClient({
  ...
  const [phases, setPhases]           = useState<PhaseWithCounts[]>(initialPhases);
  const [transitions, setTransitions] = useState<TransitionWithPhases[]>(initialTransitions);
  const [msgQrData, setMsgQrData]     = useState<Record<string, MsgQrEntry[]>>(initialMsgQrData);
