...
// ── 追加メッセージスロット型 ──────────────────────────────
// 型 / EMPTY 値 / pure helper は _form-helpers.ts に切り出し (vitest が
// bracket route パスの .tsx 解析でエラーになるため)。
// _form.tsx 側は import + re-export で公開 API を維持する。
...
// ── コンバーター ──────────────────────────────────────────
// msgToAdditionalSlot / additionalSlotToMsgBody は _form-helpers.ts に移動
// (ファイル冒頭で re-export 済み)。
...
// MessageFormState (1 通目) と AdditionalMessageSlot (2 通目以降) はいずれも
// この共通形を満たすため、TimingConfigSection は両方で使い回せる。
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    for (const [k, v] of Object.entries(vals)) {
                      // k はプリセットの timing キーで TimingFormFields に含まれる
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "12px 14px" }}>
        ...
        {/* 演出設定 (既読 / typing / loading) — 折りたたみ。1 通目と同じ TimingConfigSection を再利用 */}
        <TimingConfigSection
          form={slot}
