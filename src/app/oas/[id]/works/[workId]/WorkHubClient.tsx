...
type WorkHubClientProps = {
  oaId: string;
  workId: string;
  initialOaTitle: string;
  initialWork: WorkListItem | null;
  initialPhaseCount: number;
  initialTransCount: number;
...
export function WorkHubClient({
  ...
  const { role } = useWorkspaceRole(oaId, initialWorkspaceRole);
  const { maxWorks, planDisplayName, planName } = useWorkLimit(oaId);

  const oaTitle = initialOaTitle;
  const work = initialWork;
  const phaseCount = initialPhaseCount;
  const transCount = initialTransCount;
  const loading = false;
