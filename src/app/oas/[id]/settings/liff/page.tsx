...
// OA 単位の LIFF 設定。
//  - LIFF ID（OA 専用）の設定（owner のみ / 未設定時は NEXT_PUBLIC_LIFF_ID フォールバック）
//  - LIFF URL / 実機確認用 URL + QR
//  - LINE Developers の Endpoint URL 推奨値
//  - QR 読み取り（Scan QR）を使う想定フラグ + 注意書き
//
// 権限: 編集は owner のみ（API も owner gate）。viewer/tester/admin は閲覧のみ。
...
export default function OaLiffSettingsPage() {
  ...
  const header = (
    <div className="mb-5">
      ...
      <p className="mt-1 text-[12px] text-ink-3">
        LINE アプリ内で動く LIFF の ID・実機確認用 URL・QR 読み取り設定を管理します。
      </p>
    </div>
  );
    ...
    <>
      ...
      {loading || roleLoading ? (
        ...
        <div className="flex flex-col gap-4" style={{ maxWidth: 680 }}>
          ...
          {/* ── 実機確認 URL / QR ── */}
          <div className="card" style={{ padding: "16px 20px" }}>
            <h3 className="text-[13px] font-bold text-ink mb-2">実機で確認する</h3>
          ...
          <div className="card" style={{ padding: "16px 20px" }}>
            <h3 className="text-[13px] font-bold text-ink mb-2">LINE Developers Console の設定</h3>
