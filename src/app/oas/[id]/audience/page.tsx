...
export default function AudiencePage() {
  ...
  const loadBase = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([oa, wList, segs, trks]) => {
        setOaTitle(oa.title);
        const wItems = wList.map((w) => ({ id: w.id, title: w.title }));
        setWorks(wItems);
        setSegments(segs);
        setTrackings(trks);
        if (!selectedWorkId && wItems.length > 0) setSelectedWorkId(wItems[0].id);
      })
      .catch((e) => setBaseError(e instanceof Error ? e.message : "読み込みに失敗しました"))
      .finally(() => setBaseLoading(false));
  }, [oaId, selectedWorkId]);

  const loadAnalytics = useCallback(async (workId: string) => {
    if (!workId) return;
    setAnaLoading(true);
    setAnaError(null);
    try {
  ...
  const loadSegAna = useCallback(async (workId: string) => {
    if (!workId) return;
  ...
  useEffect(() => { loadBase(); }, [oaId]);

  useEffect(() => {
    if (!selectedWorkId) return;
    loadAnalytics(selectedWorkId);
    loadSegAna(selectedWorkId);
  }, [selectedWorkId]);

  // ── Auto-refresh ─────────────────────────────────────────────────────────
  useEffect(() => {
    if (autoRefresh && selectedWorkId) {
      intervalRef.current = setInterval(() => loadAnalytics(selectedWorkId), 30000);
    } else {
      if (intervalRef.current) clearInterval(intervalRef.current);
    }
    return () => { if (intervalRef.current) clearInterval(intervalRef.current); };
  }, [autoRefresh, selectedWorkId, loadAnalytics]);

  // ── Handlers ─────────────────────────────────────────────────────────────
  function switchTab(tab: TabType) { router.push(`/oas/${oaId}/audience?tab=${tab}`); }
  ...
  function WorkSelector({ showRefresh = true }: { showRefresh?: boolean }) {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {baseLoading ? (
          ...
          <p style={{ fontSize: 13, color: "#9ca3af" }}>
            作品がありません。<Link href={`/oas/${oaId}/works/new`} style={{ color: "#06C755", marginLeft: 4 }}>作品を追加 →</Link>
          </p>
        ) : (
        ...
        {showRefresh && selectedWorkId && (
          <button onClick={() => { loadAnalytics(selectedWorkId); loadSegAna(selectedWorkId); }}
            className="btn btn-ghost" disabled={anaLoading} style={{ fontSize: 12, padding: "6px 14px" }}>
  ...
  function AnaErrorBanner() {
    ...
    return (
      <div className="alert alert-error" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span>{anaError}</span>
        <button onClick={() => selectedWorkId && loadAnalytics(selectedWorkId)}
    ...
    <>
      ...
      {activeTab === "data" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {/* プレイ時間統計 */}
          <div className="card">
            <div style={{ fontWeight: 700, fontSize: 14, color: "#111827", marginBottom: 14 }}>⏱ プレイ時間統計</div>
            {anaLoading ? (
          ...
          <div className="card" style={{ padding: 0 }}>
            <div style={{ padding: "14px 20px 10px", borderBottom: "1px solid #e5e5e5" }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: "#111827" }}>📋 問題別分析</div>
            ...
            {anaLoading ? (
              ...
              <div style={{ padding: "24px 20px", textAlign: "center", color: "#9ca3af", fontSize: 13 }}>
                フェーズが登録されていません。<Link href={`/oas/${oaId}/works`} style={{ color: "#06C755", marginLeft: 4 }}>作品を編集 →</Link>
              </div>
            ) : (
      ...
      {activeTab === "realtime" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <WorkSelector showRefresh={false} />
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              {selectedWorkId && (
                <button onClick={() => loadAnalytics(selectedWorkId)} className="btn btn-ghost"
          ...
          {/* フェーズ別現在地 */}
          <div className="card">
            <div style={{ fontWeight: 700, fontSize: 14, color: "#111827", marginBottom: 14 }}>📍 フェーズ別現在地</div>
            {anaLoading ? (
      ...
      {activeTab === "flow" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            ...
            {selectedWorkId && (
              <Link
                href={`/oas/${oaId}/works/${selectedWorkId}/scenario`}
          ...
          <div className="card" style={{ padding: 0 }}>
            <div style={{ padding: "14px 20px 10px", borderBottom: "1px solid #e5e5e5" }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: "#111827" }}>📍 フェーズ別進行分析</div>
              <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>各フェーズへの到達・クリア・離脱の状況</div>
            </div>
            {anaLoading ? (
              ...
              <div>
                {analytics.phase_stats.map((ps, idx) => {
                  const total   = analytics.summary.total_players;
                  const maxR    = Math.max(...analytics.phase_stats.map((p) => p.reached), 1);
                  const reachPct  = total > 0 ? Math.round((ps.reached / total) * 100) : 0;
                  const barPct    = Math.round((ps.reached / maxR) * 100);
                  const dropPct   = ps.reached > 0 ? Math.round((ps.dropped_out / ps.reached) * 100) : 0;
                  return (
                    <div key={ps.phase_id} style={{
                      padding: "14px 20px",
                      borderBottom: idx < analytics.phase_stats.length - 1 ? "1px solid #f3f4f6" : "none",
                    }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                        <span style={{ fontWeight: 600, fontSize: 14, color: "#111827" }}>{ps.phase_name}</span>
                        {selectedWorkId && (
                          <Link
                            href={`/oas/${oaId}/works/${selectedWorkId}/phases/${ps.phase_id}`}
                            style={{ fontSize: 11, color: "#6b7280", textDecoration: "none", background: "#f3f4f6", borderRadius: 4, padding: "1px 8px" }}
                          >
                            編集
                          </Link>
                        )}
                      ...
                      <div style={{ marginBottom: 8 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#6b7280", marginBottom: 3 }}>
                          ...
                          <span style={{ fontWeight: 700, color: "#374151" }}>
                            {ps.reached}人 <span style={{ fontWeight: 400, color: "#9ca3af" }}>(全体の {reachPct}%)</span>
                          </span>
                        </div>
                        <div style={{ height: 8, background: "#f3f4f6", borderRadius: 4, overflow: "hidden" }}>
                          <div style={{ height: "100%", borderRadius: 4, background: "#2563eb", width: `${barPct}%`, transition: "width .4s" }} />
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        {[
                          ...
                          <span key={label} style={{
                            ...
                            <strong>{label}</strong>
                            <span style={{ fontWeight: 700 }}>{value}人</span>
          ...
          {!anaLoading && analytics && analytics.dropout_distribution.length > 0 && (
            <div className="card">
              <div style={{ fontWeight: 700, fontSize: 14, color: "#111827", marginBottom: 14 }}>🚪 フェーズ別離脱分布</div>
          ...
          <div style={{
            ...
            🧭 <strong>分岐ごとの通過率</strong>（正解ルート vs 不正解ルートの選択割合など）は今後追加予定です。
            遷移の設計変更は
            {selectedWorkId ? (
              <Link href={`/oas/${oaId}/works/${selectedWorkId}/scenario`} style={{ color: "#2563eb", marginLeft: 4 }}>
                シナリオフロー →
              </Link>
            ) : " シナリオフロー"} から行ってください。
          </div>
      ...
      {activeTab === "segments" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ padding: "12px 16px", background: "#f9fafb", borderRadius: 10, border: "1px solid #e5e7eb" }}>
            <WorkSelector showRefresh={true} />
            {!selectedWorkId && <p style={{ fontSize: 12, color: "#9ca3af", marginTop: 6 }}>作品を選択するとセグメントごとの分析データが表示されます</p>}
          </div>
                ...
                return (
                  <div key={seg.id} className="card" style={{ padding: "16px 20px" }}>
                    ...
                    <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: 12 }}>
                      {!selectedWorkId ? (
                        <p style={{ fontSize: 12, color: "#9ca3af" }}>作品を選択すると分析データが表示されます</p>
                      ) : segAnaLoading ? (
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                          ...
                          <span style={{
                            ...
                            background: trk.utm_enabled ? "#dbeafe" : "#f3f4f6",
                            color: trk.utm_enabled ? "#1d4ed8" : "#6b7280",
                          }}>
...
function KpiCard({
  ...
  return (
    <div className="card" style={{ textAlign: "center", padding: "18px 16px" }}>
      ...
      {note && !loading && (
        <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 5 }}>{note}</div>
      )}
    </div>
  );
}
