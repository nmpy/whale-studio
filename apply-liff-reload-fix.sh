...
# page.tsx の onCloseEdit から不要な reload() を外す（LiffBlockItem 側で復元するため）
python3 <<'PY'
from pathlib import Path

p = Path("src/app/oas/[id]/works/[workId]/liff/page.tsx")
s = p.read_text(encoding="utf-8")

old = 'onCloseEdit={() => { liff.setEditingBlockId(null); liff.reload(); }}'
new = 'onCloseEdit={() => liff.setEditingBlockId(null)}'

if old in s:
    p.write_text(s.replace(old, new), encoding="utf-8")
    print("patched onCloseEdit")
elif new in s:
    print("onCloseEdit already patched")
else:
    raise SystemExit("target onCloseEdit pattern not found")
PY
